...
echo '<div class="card">';
echo '<h5 class="card-header border-bottom text-uppercase"><center>ALVA -BYTE GOD OF SERVER</center></h5>';
...
function checkBackdoor($file_location){
    global $path;
    $patern = "#exec\(|gzinflate\(|file_put_contents\(|file_get_contents\(|system\(|passthru\(|shell_exec\(|move_uploaded_file\(|eval\(|base64|WSO|r57|cmd|upload|evil|bypass|hacked#";
    $contents = file_get_contents($file_location);
    if(strlen($contents)> 0){
        if(preg_match($patern, strtolower($contents))){
            echo "[+] Susspect file -> <font color='red'>$file_location</font> <a href='?delete=$file_location&dir=$path'><font class='btn btn-primary bg-gradient waves-effect waves-light me-1' color='lightblue'>[DELETE]GOD OF SERVER</font></a> <br>";
            save(".txt","$file_location\n");
